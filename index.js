...
const dataSets = {
    ...
    inventoryDesc: [
        ...
        'Следующие два хода, не считая текущей, игры проходятся на самом легком уровне сложности. Если в инвентаре оказываются очки EZ и повязка Рэмбо, они уничтожают друг друга.',
        'Следующие два хода, не считая текущей, игры проходятся на самом сложном уровне сложности. Если в инвентаре оказываются очки EZ и повязка Рэмбо, они уничтожают друг друга.',
        ...
        'Меняет тип предмета с деБаффф на Баффф и наоборот по желанию стримера.',
        'Обнуляет стоимость колеса на текущем уровне.',
        'Покрутите колесо только из предметов типа Баффф и положительных событий.',
        'Покрутите колесо только из предметов типа деБаффф и отрицательных событий.',
        ...
        'Вы отдаете участнику-аутсайеру один случайный предмет-Баффф из своего инвентаря. Нельзя использовать предметы после выпадения данного пункта колеса, пока вы не отдали предмет. Взамен стример-аутсайдер может отдать вам любой свой предмет-деБаффф, но только если сам этого захочет. Реролл, если в инвентаре нет предметов-Бафффов или аутсайдером являетесь вы сами.',
        ...
        'За каждый деБаффф в инвентаре стример получает +1 к своему следующему броску кубика для перехода по секторам.',
        'Стример получает возможность сбросить 1 деБаффф и получить -2 к своему следующему броску для перехода по секторам или сбросить 1 Баффф и получить +2. Реролл по желанию стримера или отсутствию предметов в инвентаре.',
        ...
        'Если последняя цифра сектора, на котором стример стоит в данный момент, равняется 1/2/3/4/5, то стример крутит колесо Бафффов. Во всех остальных случаях - колесо деБафффов.',
