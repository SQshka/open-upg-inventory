...
const dataSets = {
    ...
    inventoryDesc: [
        ...
        'При использовании произведите реролл игры. Использование свитка позволяет избавиться от эффектов, влияющий на ролл колеса (увеличение/уменьшение диапазона часов и т.д.) по желанию стримера.',
        ...
        'При каждом ролле колеса приколов тратится заряд. Колеса стоят фиксированно 1500 рублей пока предмет находится в инвентаре. После сброса предмета стоимость возвращается к тому значению, которое было до выпадения этого предмета. Повторное выпадение руки, пока она в инвентаре, запрещено. (Пример. колесо было 500р -> выпала рука -> три покрута -> цена колеса становится 1000р)',
        ...
        'Защищает стримера на текущий и следующий ход от ловушек-событий. Ловушки-предметы все также можно бросать в стримера под тотемом.',
        ...
        'Если клетка на части игрового поля где стоит стример, четная или угловая, то он крутит колесо баффов. Если нечетная - колесо дебаффов. Пример отсчета: Старт(0), Каловар (1) (нечет), Подлянка/Кайфарик(2) (чет) и т.д.',
    ...
    effectsDesc: [
        ...
        'Вы знатно обкекались. При дропе можете использовать заряд. Вместо перемещения на клетку "Тюрьма", вернитесь на клетку, на которой вы были на прошлом ходу. Если там нужно проходить игру, придется это сделать (за поинты, естественно). Не может заблокировать эффект от Дырявого парашюта.',
        'При каждом кручении колеса приколов замените один неугодный вам пункт на тот, который вам нужен. Озвучивать замену нужно перед каждым роллом. Если колесо уже крутится, а стример не озвучил замену, то сделать это уже нельзя.',
        ' нЗащищает стримераа текущий и следующий ход от ловушек-событий. Ловушки-предметы все также можно бросать в стримера под тотемом.', 
        'Если при броске двух кубиков на движение значение обоих одинаковое, то получите +2 к итоговому значению броска. Если выпадает две 6, то вместо + к движению, получите +1 поинт.',
        'Произведите реролл колеса. Выполните два соседних пункта от выпавшего. Порядок: сначала верх, потом низ.',
        'Меняет тип предмета с дебафф на бафф и наоборот по желанию стримера.',
